...
<div class="wrap">
	...
	if(@$_POST['np_create_news'])
	{
		...
		$postdate=$_POST['np_post_date'];
		$disab=$_POST['np_status'];
		$sort_order=$_POST['np_sort_order'];
		...
		else
		{
			...
			$data['content']=esc_sql($_POST['np_content']);
			$data['title']=esc_sql($_POST['np_title']);
			$data['sort_order']=esc_sql($_POST['np_sort_order']);
			$data['category']=$_POST['np_category'];
			if($_POST['np_post_date']=="")
				$data['post_date']=date('Y-m-d',time());
			else
				$data['post_date']=$_POST['np_post_date'];

			$data['post_time']=time();
			$data['url']=esc_sql($_POST['np_url']);
	...
	<form method="post" name="news-add" enctype="multipart/form-data" onsubmit="return np_validate_add_news();">
		<table border="0"  class="np_table">
			...
			<tr>
				...
				<td>
					Title<br>
					<input name="np_title" value="<?php  echo $title;?>" type="text" id="np_title" style="width:80%; padding:5px;">
					<br>
			...
			<tr>
				...
				<td>
					...
					<?php  wp_editor("$content", "np_content", "", false);?>
					<br>
			...
			<tr>
				...
				<td>
					News Post Date<br>
					<input name="np_post_date"  value="<?php  echo $postdate;?> " id="np_post_date" type="text" class="tcal" > 
			...
			<tr>
				<td>
					Sort Order<br>
					<input name="np_sort_order"  value="<?php  echo $sort_order; ?>"  type="number" class="tcal" > 
